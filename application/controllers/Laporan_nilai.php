...
class Laporan_nilai extends CI_Controller
{
	...
	function nilai_semester()
	{
		...
		$pdf->Cell(190, 7, 'SMAS NURUL YAQIN', 1, 1, 'C');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(190, 5, 'JL. MATAHARI RT 04 RW 01 DESA RAWA MEDANG', 1, 1, 'C');
		...
		$pdf->SetFont('Arial', '', 9);
		$sqlMapel = "SELECT tj.id_jadwal,tj.kd_jurusan,tm.nama_mapel 
	                    FROM tbl_jadwal as tj,tbl_mapel as tm
	                    WHERE tj.kd_mapel=tm.kd_mapel and tj.kd_jurusan='" . $siswa['nama_jurusan'] . "' and tj.kd_tingkatan=" . $siswa['kd_tingkatan'] . " GROUP BY tm.nama_mapel";
