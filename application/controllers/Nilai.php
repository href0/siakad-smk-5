...
class Nilai extends CI_Controller
{
	...
	function update_nilai()
	{
		...
		$checkJadwal = $this->db->get_where('tbl_jadwal', ['id_jadwal' => $idjadwal])->row_array();
		$getJadwalByMapel = $this->db->get_where('tbl_jadwal', ['kd_mapel' => $checkJadwal['kd_mapel']])->result_array();
		...
		if ($check->num_rows() > 0) {
			...
			foreach ($getJadwalByMapel as $row) {
