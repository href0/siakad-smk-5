	...
	function check_login()
	{
		if (isset($_POST['submit'])) {
			...
			if (!empty($loginUser)) {
				...
				// $sessionGuru digunakan untuk mengkonversi data agar sesuai dengan data yang ada di tbl_user, sbg contoh di tbl_user ada nama_lengkap sedangkan di tbl_guru hanya ada nama_guru maka dari itu kita menmbuat 'nama_lengkap' yang mana datanya diambil dari $loginSiswa['nama_guru']
				$sessionSiswa = array(
					'nama_lengkap'   => $loginSiswa['nama'],
					'id_level_user'  => 5,
					'nim'			=> $loginSiswa['nim'],
					'kd_kelas'  => $loginSiswa['kd_kelas'],
					'username_siswa'		 => $loginSiswa['username'],
				);
				// $this->session->set_userdata($sessionSiswa); -> maksudnya mengset userdata yang mana datanya diambil dari $sessionSiswa
